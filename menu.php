							...
							if($rparameters['meta_state']==1 && $rright['side_your_meta']!=0)
							{
								...
								if($_GET['userid']!='%' && $_GET['state']=='meta') {echo '<li class="nav-item active">';} else {echo '<li class="nav-item">';} echo "
									<a class=\"nav-link\" title=\"$label_meta\" href=\"./index.php?page=dashboard&amp;userid=$_SESSION[user_id]&amp;state=meta&amp;ticket=%25&amp;category=%25&amp;subcat=%25&amp;title=%25&amp;date_create=%25&amp;priority=%25&amp;criticality=%25&amp;company=%25\">
										<i class=\"nav-icon fa fa-angle-right pr-1\"><!----></i>
										"; echo T_('À traiter '); echo "($cntmeta[0])
									</a>
								</li>";
							...
							if($rparameters['ticket_observer'] && $rright['side_your_observer'])
							{
								...
								if($ticket_observed['counter']>0)
								{
									if($_GET['view']=='observer') echo '<li class="nav-item active">'; else echo '<li class="nav-item">'; echo '
										<a class="nav-link" title="'.T_("Tickets sur lesquels vous êtes positionné en tant qu'observateur").'" href="./index.php?page=dashboard&amp;userid='.$_SESSION['user_id'].'&amp;view=observer">
											<i class="nav-icon fa fa-angle-right pr-1"><!----></i>
							...
							if($rright['admin'])
							{
								...
								if($_GET['page']=='admin' && $_GET['subpage']=='system') {echo '<li class="nav-item active" >';} else {echo '<li class="nav-item" >';} echo '
									<a class="nav-link" href="./index.php?page=admin&amp;subpage=system">
										...
										'.T_('Système').'
										';
										if($rparameters['system_error']) {echo '&nbsp;&nbsp;<span title="'.T_("Des erreurs importantes sont détectées, corriger tous les points de couleur rouge").'" class="fas fa-exclamation-triangle text-110 text-danger-m2"><!----></span>';}
										echo '
									</a>
								</li>';
								if($rparameters['log'])

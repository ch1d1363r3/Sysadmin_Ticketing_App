...
if(!extension_loaded('pdo_mysql')) {echo 'ERROR : PHP pdo_mysql extension missing';exit;}
...
if($_GET['page']!='register' && $_GET['page']!='forgot_pwd' && $_SESSION['user_id']){session_name(md5_file('connect.php'));}
session_start();
...
//check php version
if(phpversion()<7) {echo DisplayMessage('error', T_('Version de PHP non supportée '.phpversion().', requis 7.3 min'));};
...
<html lang="fr" style="--scrollbar-width:17px; --moz-scrollbar-thin:17px; font-size: 0.925rem;">
	<head>
		...
		if(($_GET['page']=='ticket') || ($_GET['page']=='asset')|| ($_GET['page']=='test') || ($_GET['page']=='dashboard' && $_GET['view']=='activity')) 
		{
			echo '
			...
			if(($_GET['page']=='ticket') || ($_GET['page']=='asset') || ($_GET['page']=='dashboard') || ($_GET['page']=='admin/user') || ($_GET['subpage']=='user')) 
			{
				...
				<script>
					if($(".chosen-select"))
						...
						';
						//specific parameters
						if($_GET['page']=='dashboard'){echo '$(\'.chosen-drop\').css({width:"150px"});';} 
						if($_GET['page']=='admin/user'){echo '$(\'.chosen-results\').css("font-size","13px");';} 
						if($_GET['page']=='admin/user'){echo '$(\'.chosen-single\').css("font-size","13px");';} 
						if($_GET['page']=='admin/user'){echo '$(\'.chosen-single\').css({height:"32px"});';} 
						//common parameters
						echo '$(\'.chosen-single\').css("background-color","#FFF");';
						echo '$(\'.chosen-results\').css("background-color","#FFF");';
						echo '
				</script>
				';
			}

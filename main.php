		...
		<div class="navbar-inner">
			<div class="navbar-intro justify-content-xl-between">
				...
				<a class="navbar-brand text-white" target="_blank" title="'.T_('Ouvre un nouvel onglet sur le site de GestSup').'" href="https://gestsup.fr">
					...
					<!-- <i class="fa fa-ticket-alt text-80" style="ms-transform:rotate(45deg); webkit-transform:rotate(45deg); transform:rotate(45deg);"> -->
					'; 
					if($mobile) 
					{
				...
				)
				{
							...
							if($check_ticket_user_sender[0]!=$_SESSION['user_id'])
							{
								...
								if($service_check==1) {include("$_GET[page].php");}
									...
									if($rright['side_all_agency_edit'] && $cnt_agency) {
										...
										$msg_error=T_("Vous n'avez pas les droits d'accès pour modifier le ticket de ce service, contacter votre administrateur");
									}
								}
							} else {
					...
					elseif($_GET['companyview']==1 && $rparameters['user_company_view']==1 && $rright['side_company']!=0 && $ruser['company']!=0)	
					{
						if($_GET['page']=='ticket' && $_GET['id'] && $_GET['action']!='template')
						{
							...
							if(($check_ticket_company['company']==$ruser['company']) && ($ruser['company']!=0))
							{
								...
								$msg_error=T_("Vous n'avez pas les droits de consulter ce ticket, contacter votre administrateur (ERREUR 1)");
							}
					...
					elseif($_GET['view']=='observer' && $rparameters['ticket_observer'] && $rright['side_your_observer'] && $_GET['page']=='ticket' && $_GET['id'])	
					{
						...
						if(!empty($check_ticket_observer))
						{
							...
							$msg_error=T_("Vous n'avez pas les droits de consulter ce ticket, contacter votre administrateur (ERREUR 2)");
						}
					}

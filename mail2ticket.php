...
if(php_sapi_name() != "cli")
{
	...
	{
		//check token
		$qry=$db->prepare("SELECT `id` FROM `ttoken` WHERE `action`='mail2ticket' AND `token`=:token");
		...
		foreach ($tab_MailsInfos as $tab_MailsInfo){
			if($tab_MailsInfo->seen==0)
			{
				...
				if($rparameters['server_timezone'])
				{
					...
					if($rparameters['debug']) {echo '['.$mailbox.'] [mail '.$count.'] System date : '.$datetime.'<br />';}
				} else {
					$datetime = $mail->date;
					if($rparameters['debug']) {echo '['.$mailbox.'] [mail '.$count.'] Mail date : '.$datetime.'<br />';}
				}
				...
				else
				{
					...
					if($find_ticket_number && $rparameters['imap_reply']) 
					{
						...
						$qry=$db->prepare("INSERT INTO `tincidents` (`user`,`technician`,`title`,`description`,`date_create`,`date_modif`,`techread`,`state`,`criticality`,`disable`,`place`,`creator`) 
						VALUES (:user,:technician,:title,'',:date_create,:date_modif,'0',:state,'4','0','0',:creator)");
						$qry->execute(array('user' => $user_id,'technician' => $tech,'title' => $subject,'date_create' => $datetime,'date_modif' => $datetime,'state' => $rparameters['ticket_default_state'],'creator' => $user_id));

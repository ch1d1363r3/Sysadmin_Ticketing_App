			...
			<div class="cards-container" id="card-container-'.$project['id'].'">
				<div class="card bcard shadow" id="card-'.$project['id'].'">
					...
					<div class="card-body p-0">
						<div class="pt-3 pb-3">
							';
							//check number task lines for this project
							$qry2=$db->prepare("SELECT ROUND(COUNT(`id`)/10) AS `lines` FROM `tprojects_task` WHERE project_id=:project_id");
							$qry2->execute(array('project_id' => $project['id']));
							$task=$qry2->fetch();
							$qry2->closeCursor();
							...
							if($task['lines']==2)
							{
								...
								<div id="smartwizard-1" class="sw-main sw-theme-circles">
									<ul class="mx-auto nav nav-tabs step-anchor">
										';
										$qry2=$db->prepare("SELECT `number`,`ticket_id` FROM `tprojects_task` WHERE `project_id`=:project_id ORDER by `number` LIMIT 0,10");
										$qry2->execute(array('project_id' => $project['id']));
										while($task=$qry2->fetch()) 
										{
											...
											<li data-target="" class="nav-item '.$class.'" >
												<a class="nav-link" >
													<span class="step-title" title="'.T_('Numéro de la tâche').'" >
														<span class="text-secondary-d2">'.$task['number'].'</span>
													</span>
												...
												<div class="step-description">
													...
													<div class="p-0"></div>
													<i title="'.$date_label.'" class="fa fa-calendar text-'.$color.'"><!----></i> <span class="text-secondary-d2">'.$date.'</span>
												</div>
								...
								echo '
								<div id="smartwizard-2" class="sw-main sw-theme-circles">
									<ul class="mx-auto nav nav-tabs step-anchor">
										';
										$qry2=$db->prepare("SELECT `number`,`ticket_id` FROM `tprojects_task` WHERE `project_id`=:project_id ORDER by `number` LIMIT 10,20");
										$qry2->execute(array('project_id' => $project['id']));
										while($task=$qry2->fetch()) 
										{
							...
							{
								echo '
								<div id="smartwizard-3" class="sw-main sw-theme-circles">
									<ul class="mx-auto nav nav-tabs step-anchor">
										';
										$qry2=$db->prepare("SELECT `number`,`ticket_id` FROM `tprojects_task` WHERE `project_id`=:project_id ORDER by `number` LIMIT 0,10");
										$qry2->execute(array('project_id' => $project['id']));
								...
								echo '
								<div id="smartwizard-4" class="sw-main sw-theme-circles">
									<ul class="mx-auto nav nav-tabs step-anchor">
										';
										$qry2=$db->prepare("SELECT `number`,`ticket_id` FROM `tprojects_task` WHERE `project_id`=:project_id ORDER by `number` LIMIT 10,20");
										$qry2->execute(array('project_id' => $project['id']));
								...
								echo '
								<div id="smartwizard-5" class="sw-main sw-theme-circles">
									<ul class="mx-auto nav nav-tabs step-anchor">
										';
										$qry2=$db->prepare("SELECT `number`,`ticket_id` FROM `tprojects_task` WHERE `project_id`=:project_id ORDER by `number` LIMIT 20,30");
										$qry2->execute(array('project_id' => $project['id']));
										while($task=$qry2->fetch()) 
										{
								...
								echo '
								<div id="smartwizard-'.$project['id'].'" class="sw-main sw-theme-circles">
									<ul class="mx-auto nav nav-tabs step-anchor">
										';

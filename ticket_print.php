...
//secure id
if(!is_numeric($_GET['id'])) {echo 'ERROR : incorrect id'; exit;}
...
//load token table
$qry=$db->prepare("SELECT `token` FROM `ttoken` WHERE `ticket_id`=:ticket_id AND `action`='ticket_access' AND `user_id`=:user_id");
$qry->execute(array('ticket_id' => $_GET['id'], 'user_id' => $_SESSION['user_id']));
$token=$qry->fetch();
$qry->closeCursor();
...
//secure access
if($token && $_GET['token']==$token['token'] && $rright['ticket_print'])
{

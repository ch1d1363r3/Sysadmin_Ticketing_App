		...
		<div class="page-header position-relative">
			<h1 class="page-title text-primary-m2" >
				...
				<small class="page-info text-secondary-d2">
					<i class="fa fa-angle-double-right mr-2"><!----><!----></i>'.T_('Nombre').' : '.$r1[0].' '.T_('Activés et').' '.$r2[0].' '.T_('Désactivés').'
				</small>
			</h1>
		</div>';
	}
			...
			<div class="card bcard shadow">
				<div class="card-header">
					...
					<span class="card-toolbar">
						...
						echo '
						<button value="modify" id="modify2" name="modify" type="submit" form="1" class="btn btn-success ml-1">
				...
				<div class="card-body">
					<div class="card-main no-padding">
						<form id="1" name="1" method="POST" action="" class="form-horizontal">
                                <fieldset>
                                <div class="col-sm-12">
                            		<div class="tabs-above">
										...
										';
                    								...
                    								if($rright['admin_user_view'])
                    								{
                    									echo '
                    										...
                    										<div class="space-4"></div>';
                    											...
                    											echo '
                    												'.T_('Catégorie').' :
                    												<select style="width:auto;" class="form-control form-control-sm d-inline-block mt-2" name="category" onchange="submit()" >
                    													<option value="%"></option>';
																	...
																	echo '
                    												<div class="space-4"></div>
                    												Nom : <input style="width:auto;" class="form-control form-control-sm d-inline-block mt-2" autocomplete="off" name="viewname" type="text" value="'.$_POST['name'].'" size="20" />';
                    										    ...
                    										    echo '
                        										    ...
                        										    <div class="space-4"></div>
                        										    <select style="width:auto; max-width:235px;" class="form-control form-control-sm d-inline-block" name="default_ticket_state">
                    										    ...
                    										    echo '
                        										    ...
                        										    <div class="space-4"></div>
                        										    <select style="width:auto; max-width:235px;" class="form-control form-control-sm d-inline-block" name="dashboard_ticket_order">
                        										        <option '; if($user1['default_ticket_state']==''){echo "selected";} echo ' value="">'.T_("Aucun (Géré par l'administrateur)").'</option>
											...
											<div id="infos" class="tab-pane'; if($_GET['tab']=='infos' || $_GET['tab']=='') echo 'active'; echo '">
                								...
                								if($rright['admin']) {$label_hide='';$input_type='type="text"';} else {$label_hide='hidden';$input_type='type="hidden"';}
                                                echo '
...
else if($_GET['action']=="add" && (($_SESSION['user_id']==$_GET['userid']) || $rright['admin']))
{
	//remove '' to display
	...
	echo '
		<div class="col-12 cards-container">
			<div class="card bcard shadow">
				...
				<div class="card-body">
					<div class="card-main no-padding">
						<form id="1" name="1" method="POST"  action="">
							<fieldset>
								<label for="firstname">'.T_('Prénom').' :</label>
							...
							<div class="border-t-1 brc-secondary-l1 bgc-secondary-l3 py-3 text-center mt-5">
								<button title="'.T_('CTRL+S').'" value="add" id="add" name="add" type="submit" class="btn btn-success mr-2">
...
}elseif($_GET['action']=="delete" && $rright['admin'] && $_GET['userid'])
{
	...
	$qry->execute(array('delete_user' => $delete_user,'observer1' => $_GET['userid']));
	$qry=$db->prepare("UPDATE `tincidents` SET `observer2`=:delete_user WHERE `observer2`=:observer2");
	$qry->execute(array('delete_user' => $delete_user,'observer2' => $_GET['userid']));
	$qry=$db->prepare("UPDATE `tincidents` SET `observer3`=:delete_user WHERE `observer1`=:observer3");
	$qry->execute(array('delete_user' => $delete_user,'observer3' => $_GET['userid']));
	//update threads
...
elseif(($_GET['action']=='edit') && ($_GET['userid']!='') && !$rright['admin'])
{
   echo '
	<div role="alert" class="alert alert-lg bgc-danger-l3 border-0 border-l-4 brc-danger-m1 mt-4 mb-3 pr-3 d-flex">
		<div class="flex-grow-1">
						...
						while ($row = $qry->fetch()) 
						{
							...
							echo '
								<tr class="bgc-h-orange-l4">
									<td class="text-secondary-d2 text-95 text-600" onclick=\'window.location.href="index.php?page=admin&amp;subpage=user&amp;action=edit&amp;userid='.$row['id'].'&amp;tab=infos&amp;disable='.$_GET['disable'].'";\' >
										...
										'.mb_strtoupper($row['lastname']).' '.ucfirst(mb_substr($row['firstname'],0,1)).mb_strtolower(mb_substr($row['firstname'],1)).' 
									</td>
									<td class="text-dark-m3" onclick=\'window.location.href="index.php?page=admin&amp;subpage=user&amp;action=edit&amp;userid='.$row['id'].'&amp;tab=infos&amp;disable='.$_GET['disable'].'";\' >'.$row['login'].'</td>
									';
									if($rparameters['user_advanced']==1) {
										//get company name
										...
										$row2=$qry2->fetch();
										$qry2->closeCursor();
										echo '<td class="text-dark-m3" onclick=\'window.location.href="index.php?page=admin&amp;subpage=user&amp;action=edit&amp;userid='.$row['id'].'&amp;tab=infos&amp;disable='.$_GET['disable'].'";\' >'.$row2['name'].'</td>';
									}
									if($rparameters['user_agency']==1) {
										//get agencies name
										echo '<td class="text-dark-m3" onclick=\'window.location.href="index.php?page=admin&amp;subpage=user&amp;action=edit&amp;userid='.$row['id'].'&amp;tab=infos&amp;disable='.$_GET['disable'].'";\' >';
									...
									echo '
									<td class="text-dark-m3" onclick=\'window.location.href="index.php?page=admin&amp;subpage=user&amp;action=edit&amp;userid='.$row['id'].'&amp;tab=infos&amp;disable='.$_GET['disable'].'";\' >
									';
									...
									<td class="text-dark-m3" onclick=\'window.location.href="index.php?page=admin&amp;subpage=user&amp;action=edit&amp;userid='.$row['id'].'&amp;tab=infos&amp;disable='.$_GET['disable'].'";\' >'.$row['mail'].'</td>
									<td class="text-dark-m3" onclick=\'window.location.href="index.php?page=admin&amp;subpage=user&amp;action=edit&amp;userid='.$row['id'].'&amp;tab=infos&amp;disable='.$_GET['disable'].'";\' >'.$row['phone'].' '.$row['mobile'].'</td>
									<td class="text-dark-m3" onclick=\'window.location.href="index.php?page=admin&amp;subpage=user&amp;action=edit&amp;userid='.$row['id'].'&amp;tab=infos&amp;disable='.$_GET['disable'].'";\' >'.T_($r['name']).'</td>
									<td class="text-dark-m3" onclick=\'window.location.href="index.php?page=admin&amp;subpage=user&amp;action=edit&amp;userid='.$row['id'].'&amp;tab=infos&amp;disable='.$_GET['disable'].'";\' >'.$lastlogin.'</td>
									<td>
									
										<a class="action-btn btn btn-sm btn-warning mr-1" href="index.php?page=admin&amp;subpage=user&amp;action=edit&amp;userid='.$row['id'].'&amp;tab=infos&amp;disable='.$_GET['disable'].'"  title="'.T_("Modifier l'utilisateur").'" ><i style="color:#FFF;" class="fa fa-pencil-alt"><!----><!----></i></a>';

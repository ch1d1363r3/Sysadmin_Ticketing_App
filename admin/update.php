...
if($dedicated==0)
{
	...
	//open ftp connection
	$conn_id = ftp_connect($ftp_server,21,10) or die(
		DisplayMessage('error', T_('Le serveur de mise à jour').' '.$ftp_server.' '.T_('est inaccessible, vérifier votre accès Internet ou l\'ouverture de votre firewall sur le port 21'))
	);
	...
	if(!$ftp_list) {
		echo DisplayMessage('error',T_('Serveur de mise à jour inaccessible vérifier votre accès Internet et votre Firewall')); 
		logit('error', "Unable to to contact update server",$_SESSION['user_id']);
		exit;}
	//get patch only
			...
			if ($findversion==1) //version
			{
				...
				if ((!$conn_id) || (!$login_result)) {
					...
					if($cmd_update)
					{
						...
						logit('error', "Download lastest version failed",$_SESSION['user_id']);
						die;
					}
				}
				$pasv = ftp_pasv($conn_id, true);
			...
			else if ($findpatch==1) //patch
			{
				...
				if ((!$conn_id) || (!$login_result)) {
					if($cmd_update)
					{
						...
						logit('error', "Download lastest patch failed",$_SESSION['user_id']);
						die;
					}
				} 
				$pasv = ftp_pasv($conn_id, true);

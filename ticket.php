		...
		<div class="card-header">
			...
			<span class="card-toolbar">
				<?php 
					...
					{
						echo '<button type="button" style="width:31px; height:27px; padding-top:5px; margin-right:1px;" class="btn btn-xs btn-warning" title="'.T_('Créer un rappel pour ce ticket').'" data-toggle="modal" data-target="#add_event" ><span style="color:#FFF;"><i class="fa fa-bell text-120"><!----></i></span></button>&nbsp;';
					}
			...
			<div class="p-3">
				...
				<div class="form-group row <?php if((!$rright['ticket_user_disp'] && $_GET['action']!='new') || (!$rright['ticket_new_user_disp'] && $_GET['action']=='new')) {echo 'd-none';} ?>" >
					...
					<div class="col-sm-9">
						...
						<select autofocus onchange="loadVal();" style="display:inline; <?php if($mobile) {echo 'max-width:240px;';} else {if($rright['ticket_user_company']) {echo 'width:auto;';} else {echo 'max-width:269px;';}}?>" class="form-control chosen-select" id="user" name="user" <?php if(($rright['ticket_user']==0 && $_GET['action']!='new') || ($rright['ticket_new_user']==0 && $_GET['action']=='new') || $ticket_observer) {echo ' disabled="disabled" ';}?> >
							<?php
			   ...
			   <?php
								...
								<div class="col-sm-5">
									...
									if($rright['ticket_observer'])
									{
										...
										if($_POST['observer1'] && $_POST['observer2'] && $_POST['observer3'])
										{
											...
											echo '
											<select style="min-width:269px;" class="form-control col-5 d-inline-block" id="observer" name="observer" >
												';
											...
											//display add button
											echo '
											<button style="background: none; padding: 0px; border: none;" type="submit" id="add_observer">
					...
					if($rparameters['asset']==1)
					{
						if($rright['ticket_new_asset_disp'])
						{
							...
							<div class="form-group row '; if(($rright['ticket_new_asset_disp']==0 && $_GET['action']=='new') || ($rright['ticket_asset_disp']==0 && $_GET['action']!='new')) {echo 'd-none';} echo '" >
								...
								<div class="col-sm-5">
									<select style="min-width:269px;" class="form-control col-5" id="asset_id" name="asset_id" '; if(($rright['ticket_asset']==0 && $_GET['action']!='new') || ($rright['ticket_new_asset_disp']==0 && $_GET['action']=='new') || $ticket_observer) {echo 'disabled="disabled"';} echo' onchange="loadVal(); '; if($rright['ticket_asset_mandatory']) {echo 'CheckMandatory();';} echo '">
										';
										if($_POST['asset_id'])
										{
				...
				<div class="form-group row <?php if(($rright['ticket_cat_disp']==0 && $_GET['action']!='new') || ($rright['ticket_new_cat_disp']==0 && $_GET['action']=='new')) echo 'd-none';?>">
					...
					<div class="col-sm-5">
						<select <?php if($mobile) {echo 'style="max-width:105px;"';}else{echo 'style="width:auto;"';}?>  class="form-control d-inline-block mb-1 mb-md-0" title="<?php echo T_('Catégorie'); ?>" id="category" name="category" <?php if(($rright['ticket_cat']==0 && $_GET['action']!='new') || ($rright['ticket_new_cat']==0 && $_GET['action']=='new') || $ticket_observer) echo ' disabled="disabled" ';?>>
						<?php
				...
				<div class="form-group row <?php if($rright['ticket_description_disp']==0) {echo 'd-none';} ?>">
					...
					<div class="col-sm-5">
						<table id="description" border="1" width="<?php if(!$mobile) {echo '780';} else {echo '100%';}?>" style="border: 1px solid #D8D8D8;" >
							<tr>
								<td <?php if(!$rright['ticket_description']) {echo 'style="padding:5px"';} ?>>
									...
									{	
										...
										echo '<input id="text" name="text" type="hidden" value="'.htmlentities($globalrow['description']).'" />';
									}
									?>
								</td>
							</tr>
				...
				if($rright['ticket_attachment'])
				{
					...
					if($globalrow['state']==3 && empty($row[0]))
					{
						...
						<div class="form-group row">
							<div class="col-sm-2 col-form-label text-sm-right pr-0">
								<label class="mb-0" for="attachment">'.T_('Fichiers joints').' :</label>
							</div>
							<div class="col-sm-5">
				...
				<div class="form-group row <?php if(!$rright['ticket_time_disp']) {echo 'd-none';} ?>">
					...
					<div class="col-sm-5">
						<select class="form-control <?php if($mobile){echo 'col-7';} else {echo 'col-5';} ?>" id="time" name="time" <?php if(!$rright['ticket_time'] || $ticket_observer) {echo 'disabled';}?> >
							<?php
				...
				<div id="bottom_button" class="border-t-1 brc-secondary-l1 bgc-secondary-l3 py-3 text-center">
					...
					if(!$hide_button)
					{
						...
						if($rright['ticket_close'] && $_POST['state']!='3' && $globalrow['state']!='3' && $_GET['action']!='new' && $lock_tech==0 && !$ticket_observer)
						{
							echo '
						...
						if($rright['ticket_reopen']!=0 && $globalrow['state']=='3' && $_GET['action']!='new' && $lock_tech==0)
						{
							echo '

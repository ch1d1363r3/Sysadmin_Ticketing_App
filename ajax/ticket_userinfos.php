...
if(!empty($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest') 
{
	...
	//load token table
	$qry=$db->prepare("SELECT `token` FROM ttoken WHERE `ticket_id`=:ticket_id AND `user_id`=:user_id");
	$qry->execute(array('ticket_id' => $_GET['ticket'],'user_id' => $_GET['user_id']));
	$token=$qry->fetch();
	$qry->closeCursor();
	...
	if($_POST['user'] && $token['token'] && $_GET['token'] && $_GET['token']==$token['token'])
	{
		...
		//get user data
		$qry=$db->prepare("SELECT `phone`,`mobile`,`mail`,`function`,`company`,`custom1`,`custom2` FROM `tusers` WHERE id=:id");
		...
		if($user) {
			...
			echo json_encode(array("status" => "failed : error 3"));
		}
		 //close database access
		$db = null;
	} else {
		echo json_encode(array("status" => "failed : error 2"));
	} 
} else {
	echo json_encode(array("status" => "failed : error 1"));
}
?>
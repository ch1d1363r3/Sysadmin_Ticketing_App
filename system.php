...
if($_GET['subpage']=='system')
{
	if(extension_loaded('ftp'))
	{
		...
		if($connected){
			//ftp check
			$conn_id = ftp_connect('ftp.gestsup.fr',21,5) or die('ERROR : Unable to connect on GestSup FTP Server');
...
if(!isset($gestsup_version))
{
	if(strpos($changelog, $rparameters['version']))
	{
		...
		if($_GET['subpage']=='system')
		{
			...
			$ticket=$qry->fetch();
			$qry->closeCursor();
			$gestsup_version_text=' <span style="font-size:12px"><i>('.$ticket['counter'].' '.T_('Tickets').')<!----></i></span>';
		} else {
			...
			<tbody>
				<tr>
					...
					<td class="text-95 text-default-d3">
						<img src="./images/<?php echo strtolower($os); ?>_ok.png" style="border-style: none; margin-right:2px;" alt="img" /> <?php echo "<b>OS :</b> {$phpinfo['phpinfo']['System']}"; if($os=='Windows') {echo ' <i>('.T_('les environnements Windows ne sont pas recommandés pour les serveurs de production').')<!----></i>';} ?><br />
						<?php 
							echo '<img src="./images/'.$web_server_icon.'" style="border-style: none; margin-right:2px;" alt="img" /> <b>'.ucfirst($web_server_name).' :</b> '.$web_server_version.' <span style="font-size:12px"><i>('.$_SERVER['SERVER_PROTOCOL'].')<!----></i></span><br />';
						?>
						<img style="width:20px;" src="./images/<?php echo strtolower($rdb_name).'_'.$rdb_icon.'.png'; ?>" alt="img" /> 
						<?php 
							echo '<b>'.$rdb_name.' :</b> '.$rdb_version;
							echo '<span style="font-size:12px"><i> (';
							if(strtolower($rdb_name)=='mysql') {echo T_("Il est recommandé de migrer sur MariaDB, ");}
							echo T_('base').' : '.$db_name.' '.$db_size.')<!----></i><br /></span>'; 
							...
							if($php_version<'7.3.0'){
								...
								echo '<img class="pr-1" src="./images/php_ok.png" alt="img" /> <b>PHP :</b> '.phpversion();
							}
						?>  
						<br />
						<?php echo $gestsup_version; ?> <b><?php echo T_('GestSup'); ?> :</b> <?php echo $rparameters['version'].$gestsup_version_text; ?><br />
						<i style="width:16px;" class="fa fa-clock text-success"><!----></i> &nbsp;<b><?php echo T_('Horloge'); ?> :</b> <?php echo date('Y-m-d H:i:s').' <i>('.date_default_timezone_get().')<!----></i>'; ?> <br />
				...
				<tr>
					...
					<td class="text-95 text-default-d3">
						...
						if($phpinfo[$vphp]['max_execution_time'][0]>="480") echo '<i class="fa fa-check-circle text-success"><!----></i> <b>max_execution_time :</b> '.$phpinfo[$vphp]['max_execution_time'][0].'s<br />'; else echo '<i class="fa fa-exclamation-triangle text-warning"><!----></i> <b>max_execution_time : </b>'.$phpinfo[$vphp]['max_execution_time'][0].'s <i>('.T_('Valeur conseillé 480s, modifier votre php.ini relancer apache et actualiser cette page').'.)<!----></i><br />';
				...
				<tr>
					...
					<td class="text-95 text-default-d3">
						...
						echo "<br />";
						if(extension_loaded('fileinfo')) {echo '<i class="fa fa-check-circle text-success"><!----></i> <b>php_fileinfo :</b> '.T_('Activée');} else {$system_error=1; echo '<i class="fa fa-times-circle text-danger"><!----></i> <b>php_fileinfo</b> : '.T_("Désactivée").' <i>('.T_("le connecteur IMAP ne fonctionnera pas. apt install php7.3-fileinfo").')<!----></i>';}
						echo "<br />";
						if(extension_loaded('ftp')) {echo '<i class="fa fa-check-circle text-success"><!----></i> <b>php_ftp :</b> '.T_('Activée');} else {$system_error=1; echo '<i class="fa fa-times-circle text-danger"><!----></i> <b>php_ftp</b> : '.T_("Désactivée").' <i>('.T_("seule la mise à jour manuelle sera possible, dé-commenter la ligne extension=php_ftp votre php.ini'").')<!----></i>';}
						echo "<br />";
						...
						echo "<br />";
						if(extension_loaded('iconv')) {echo '<i class="fa fa-check-circle text-success"><!----></i> <b>php_iconv :</b> '.T_('Activée');} else {$system_error=1; echo '<i class="fa fa-times-circle text-danger"><!----></i> <b>php_iconv</b> : '.T_("Désactivée").' <i>('.T_("le connecteur IMAP ne fonctionnera pas. apt install php7.3-iconv").')<!----></i>';}
						echo "<br />";
						...
						echo "<br />";
						if(extension_loaded('mbstring')) {echo '<i class="fa fa-check-circle text-success"><!----></i> <b>php_mbstring :</b> '.T_('Activée');} else {$system_error=1; echo '<i class="fa fa-times-circle text-danger"><!----></i> <b>php_mbstring</b> : '.T_("Désactivée").' <i>('.T_("des erreurs sont possibles dans la liste des tickets et sur le connecteur IMAP. apt install php7.3-mbstring").')<!----></i>';}
						echo "<br />";
						...
						echo "<br />";
						if(extension_loaded('xml')) {echo '<i class="fa fa-check-circle text-success"><!----></i> <b>php_xml :</b> '.T_('Activée');} else {$system_error=1; echo '<i class="fa fa-times-circle text-danger"><!----></i> <b>php_xml</b> : '.T_("Désactivée").' <i>('.T_("le connecteur LDAP ne fonctionnera pas. apt-get install php7.3-xml").')<!----></i>';}
						echo "<br />";
				...
				<tr>
					...
					<td class="text-95 text-default-d3">
						...
						if($web_server_name=='apache') 
						{
							...
							{echo '<i class="fa fa-exclamation-triangle text-warning"><!----></i> <b>'.T_('Version Apache').' : </b>'.T_("Affichée").' <i>('.T_("pour plus de sécurité, masquez la version d'apache que vous utilisez. Passer \"ServerTokens\" à \"Prod\" dans security.conf").'.)<!----></i>';}	
							echo "<br />";
						}
						...
						{echo '<i class="fa fa-exclamation-triangle text-warning"><!----></i> <b>'.T_('Version PHP').' : </b>'.T_("Affichée").' <i>('.T_("pour plus de sécurité, masquez la version de PHP que vous utilisez. Passer le paramètre \"expose_php\" à  \"Off\" dans le php.ini").'.)<!----></i>';}	
						echo "<br />";
						...
						{echo '<i class="fa fa-exclamation-triangle text-warning"><!----></i> <b>'.T_('Durée de la session').' : </b> PHP='.$maxlifetime.'s GestSup='.$rparameters['timeout'].'m <i>('.T_("pour plus de sécurité, diminuez la durée à 24m minimum, paramètre \"session.gc_maxlifetime\" du php.ini et paramètre GestSup.").')<!----></i>';}
						echo "<br />";
						if($_GET['subpage']=='system') //not display on installation page
						{
							...
							{echo '<i class="fa fa-exclamation-triangle text-warning"><!----></i> <b>'.T_("Droits d'écriture").' : </b>'.T_('Non verrouillés').' <i>(<a target="_blank" href="https://doc.gestsup.fr/install/">'.T_('cf. documentation').'</a>).<!----></i>';}  
							echo "<br />";
							...
							{$system_error=1; echo '<i class="fa fa-times-circle text-danger"><!----></i> <b>'.T_("Répertoire installation").' : </b>'.T_('Présent').' <i>('.T_("supprimer le répertoire \"./install\" de votre serveur").').<!----></i>';}
							echo "<br />";
						}
						...
						if($_GET['subpage']=='system')
						{
							...
							} elseif($rparameters['user_password_policy'])
							{
								if($rparameters['user_password_policy_min_lenght']>=8)
								{
									...
									echo '<i class="fa fa-exclamation-triangle text-warning"><!----></i> <b>'.T_('Mots de passe').' : </b>'.T_('Longueur de mot de passe trop faible').' <i>('.T_('définir la longueur minimale à 8 caractères').').<!----></i><br />';
								}
							} else {
								echo '<i class="fa fa-exclamation-triangle text-warning"><!----></i> <b>'.T_('Mots de passe').' : </b>'.T_('Aucune politique définie').' <i>('.T_('définissez une politique de mot de passe dans Administration > Paramètres > Général > Utilisateur').').<!----></i><br />';
							}
							...
							if(!empty($user_pwd['last_pwd_chg']))
							{
								if($user_pwd['last_pwd_chg']=='0000-00-00') {
									echo '<i class="fa fa-times-circle text-danger"><!----></i> <b>'.T_('Mot de passe user').' : </b>'.T_('Pas encore modifié, ou modifié il y a trop longtemps').' <i>('.T_("changer le mot de passe du compte ayant l'identifiant user").').<!----></i><br />';
									$system_error=1;
								}
							}
							...
							if(!empty($power_pwd['last_pwd_chg']))
							{
								if($power_pwd['last_pwd_chg']=='0000-00-00') {
									echo '<i class="fa fa-times-circle text-danger"><!----></i> <b>'.T_('Mot de passe power').' : </b>'.T_('Pas encore modifié, ou modifié il y a trop longtemps').' <i>('.T_("changer le mot de passe du compte ayant l'identifiant power").').<!----></i><br />';
									$system_error=1;
								}
							}
							...
							if(!empty($super_pwd['last_pwd_chg']))
							{
								if($super_pwd['last_pwd_chg']=='0000-00-00') {
									echo '<i class="fa fa-times-circle text-danger"><!----></i> <b>'.T_('Mot de passe super').' : </b>'.T_('Pas encore modifié, ou modifié il y a trop longtemps').' <i>('.T_("changer le mot de passe du compte ayant l'identifiant super").').<!----></i><br />';
									$system_error=1;
								}
							}
							...
							if(!empty($tech_pwd['last_pwd_chg']))
							{
								if($tech_pwd['last_pwd_chg']=='0000-00-00') {
									echo '<i class="fa fa-times-circle text-danger"><!----></i> <b>'.T_('Mot de passe tech').' : </b>'.T_('Pas encore modifié, ou modifié il y a trop longtemps').' <i>('.T_("changer le mot de passe du compte ayant l'identifiant tech").').<!----></i><br />';
									$system_error=1;
								}
							}
							...
							if(!empty($admin_pwd['last_pwd_chg']))
							{
								...
								{
									$system_error=1;
									echo '<i class="fa fa-times-circle text-danger"><!----></i> <b>'.T_('Mot de passe admin').' : </b>'.T_('Pas encore modifié, ou modifié il y a trop longtemps').' <i>('.T_("changer le mot de passe du compte ayant l'identifiant admin").').<!----></i><br />';
								} else {
							...
							if(preg_match('/Debian/',$phpinfo['phpinfo']['System']))
							{
								...
								{} else {echo '<i class="fa fa-exclamation-triangle text-warning"><!----></i> <b>'.T_('Mise à jour système').' : </b>'.T_('Obsolète').' <i>('.T_("le système d'exploitation serveur n'est pas à jour. Exécuter la commande : apt update && apt upgrade").').<!----></i><br />';}
							}
						}
...
if($_GET['subpage']=='system')
{
	//update system error detection
	$qry=$db->prepare("UPDATE `tparameters` SET `system_error`=:system_error");
	$qry->execute(array('system_error' => $system_error));
}

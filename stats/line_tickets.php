...
elseif($_POST['year']=='%')
{
	...
	//open tickets
	$query2="
		SELECT year(date_create) AS x,COUNT(*) AS y 
		FROM `tincidents` 
		WHERE 
		technician LIKE '$_POST[tech]' AND
		u_service LIKE '$_POST[service]' $where_service $where_agency AND
		$where_state AND
		criticality LIKE '$_POST[criticality]' AND
		type LIKE '$_POST[type]' AND
		category LIKE '$_POST[category]' AND
		date_create NOT LIKE '0000-00-00 00:00:00' AND
		date_create LIKE '$_POST[year]-$_POST[month]-%' AND
		disable='0' 
		GROUP BY x 
		ORDER BY x
	";

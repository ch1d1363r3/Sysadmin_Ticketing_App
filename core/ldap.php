...
if($rparameters['ldap'])
{
	...
	if(($_GET['action']=='simul') || ($_GET['action']=='run') || ($_GET['ldaptest']==1) || ($_GET['ldap']==1) || ($ldap_auth==1))
	{
		...
		if($ldapbind) {
			$ldap_connection=DisplayMessage('success',T_('Connecteur opérationnel'));
		} else {
			$ldap_connection=DisplayMessage('error',T_('Le connecteur LDAP ne fonctionne pas vérifier vos paramètres').' ('.ldap_error($ldap).')');
			//log
			if($rparameters['log'])
			{
				logit('error', "LDAP : connexion failed ".ldap_error($ldap),$_SESSION['user_id']);
			}
		}
		if($ldapbind) 
		{
			...
			{		
					...
					foreach ($list_dn as $value) {
						...
						$pagesize = 100;
						$cookie = '';
						$justthese = array('samaccountname', 'userprincipalname', 'objectguid', 'uid', 'entryuuid', 'useraccountcontrol', 'givenname', 'sn', 'telephonenumber', 'mobile', 'vvdpostetel', 'streetaddress', 'postalcode', 'l', 'mail', 'company', 'facsimiletelephonenumber', 'title', 'department', 'manager');
					...
					*/
					...
					for ($i=0; $i < $cnt_ldap; $i++) 
					{
						...
						if($rparameters['debug']) echo "[DEBUG MODE] - LDAP_SamAccountName=$samaccountname LDAP_UPN=$UPN LDAP_login=$LDAP_login ldap_guid=$ldap_guid LDAP_UAC=$UAC LDAP_company=$company LDAP_department=$department";
						
						////check if account not exist in GestSup user database
						//1st check login
						$find_guid=0;
						$qry=$db->prepare("SELECT `id`,`login`,`firstname`,`lastname`, `disable`,`mail`, `phone`,`mobile`,`address1`,`zip`,`city`,`company`,`fax`,`function`,`ldap_guid` FROM `tusers`");
						$qry->execute();
						while($row=$qry->fetch()) 
						{
		...
		} elseif($_GET['subpage']=='user')
		{
			...
			//log
			if($rparameters['log']){logit('error', "LDAP : connexion failed ".ldap_error($ldap),$_SESSION['user_id']);}
		}
	} 
} else {

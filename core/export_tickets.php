...
//check db token
$qry=$db->prepare("SELECT `id`,`token` FROM `ttoken` WHERE `action`='stat_access'  AND `token`=:token AND `user_id`=:user_id");
$qry->execute(array('token' => $_GET['token'],'user_id' => $db_userid));
$token=$qry->fetch();
$qry->closeCursor();
...
if($_GET['token']==$token['token'] && $_GET['token']) 
{
	...
	if($db_state=='meta') {
		...
		state IN (SELECT id FROM tstates WHERE meta=1) AND
		$where
		disable=0
		";
	} else {
	...
	echo '<br /><br /><span style="font-size: x-large; color: red; text-align:center;"><b>'.T_("Erreur d'accès à la page, essayer de recharger la page statistique, si le problème persiste contacter votre administrateur").'.</b></span>';		
}
$db = null;
?>
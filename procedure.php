			...
			if(CheckFileExtension($file_rename)==true) {
				...
				if(move_uploaded_file($_FILES['procedure_file']['tmp_name'], $dest_folder.$file_rename)) 
				{
					...
					if(preg_match('{\<\?php}',$file_content) || preg_match('/system\(/',$file_content)) {
						...
						if($rparameters['log'] && is_numeric($_GET['id'])) {logit('security','File upload blocked on procedure '.$_GET['id'],$_SESSION['user_id']);}
					}
				} else {
	...
	echo '
		<div class="card bcard shadow" id="card-1">
			<div class="card-header">
				<h5 class="card-title">
					<i class="fa fa-book"><!----></i> '.T_("Ajout d'une procédure").'
				</h5>
			</div><!-- /.card-header -->
...
elseif($_GET['action']=='edit')
{
	...
	if($row['company_id']==$ruser['company'] || $rright['procedure_list_company_only']==0) //security check before display procedure
	{
		...
		<div class="card bcard mt-2" id="card-1">
			<div class="card-header">
				<h5 class="card-title">
					<i class="fa fa-book"><!----></i> '.T_('Procédure').' n°'.$row['id'].' : '.$row['name'].'
				</h5>
				...
				<tbody>
					';
						...
						while ($procedure=$masterquery->fetch())
						{
							...
							if(empty($rcat['name'])) {$rcat['name']='';}
							if(empty($rscat['name'])) {$rscat['name']='';}
							echo '
							<tr class="bgc-h-orange-l4" >	
								...
								<td>
									<div class="hidden-phone visible-desktop btn-group">	
										...
										if($rright['procedure_modify']) {echo'<a class="btn action-btn btn-xs btn-warning mr-1 text-center" href="./index.php?page=procedure&amp;id='.$procedure['id'].'&amp;action=edit" title="'.T_('Modifier cette procédure').'" ><span style="color:#FFF;#"><i class="fa fa-pencil-alt"><!----></i></span></a>';}
										if($rright['procedure_delete']) {echo'<a class="btn action-btn btn-xs btn-danger text-center" onClick="javascript: return confirm(\''.T_('Êtes-vous sur de vouloir supprimer cette procédure ?').'\');" href="./index.php?page=procedure&amp;id='.$procedure['id'].'&amp;action=delete" title="'.T_('Supprimer cette procédure').'" ><i class="fa fa-trash"><!----></i></a>';}
										echo '
									</div>
								</td>
							</tr>
